#!/usr/bin/env node
/**
 * pts_validate.js: health check for a generated playlist skill, plus manifest writer.
 *
 ...
 *
 * @usage
 *   node pts_validate.js --skill-dir <dir> --kb-dir <dir> [--write-manifest] [--json]
 */
const fs = require('fs');
...
if (!a.skillDir || !a.kbDir) { console.error('usage: pts_validate.js --skill-dir <dir> --kb-dir <dir> [--write-manifest] [--json]'); process.exit(2); }
...
if (!tax) E('E5', 'taxonomy.json missing');
else {
...
// Full-playlist coverage is a completion requirement, including empty extractions.
...
const scopeIds = new Set((scope.videos || []).map(v => v.id));
if (!scope.playlist_id || scope.chosen_cut !== 'all') E('E6', 'Expected an all-playlist scope');
for (const entry of scope.entries || []) {
  if (entry.id && !scopeIds.has(entry.id)) E('E6', `Playlist entry ${entry.position} missing from scope: ${entry.id}`);
}
for (const v of scope.videos || []) {
  const r = raw.videos[v.id];
  if (!r || !['fetched', 'failed', 'no-captions'].includes(r.status)) E('E6', `Pending fetch: ${v.id}`);
  if (r?.status === 'failed' || r?.status === 'no-captions') W('W4', `${v.id}: ${r.status}, ${r.reason || 'no readable captions'}`);
  if (r?.status !== 'fetched') continue;
  const extraction = read(path.join(a.kbDir, 'extractions', `${v.id}.json`));
  if (!extraction) { E('E6', `Missing extraction: ${v.id}`); continue; }
  let x;
  try { x = JSON.parse(extraction); } catch { E('E6', `Invalid extraction JSON: ${v.id}`); continue; }
  if (x.video_id !== v.id || !Array.isArray(x.concepts)) E('E6', `Invalid extraction: ${v.id}`);
  else if (!x.concepts.length && !x.no_concepts_reason) E('E6', `Empty extraction needs a reason: ${v.id}`);
  else if (x.concepts.length && !(tax?.concepts || []).some(c => c.video_ids?.includes(v.id))) E('E6', `Teaching omitted from taxonomy: ${v.id}`);
}
for (const f of ['glossary.md', 'patterns.md', 'cheatsheet.md', 'sources.md']) {
  if (!files[f]) E('E6', `Missing support file: ${f}`);
}
...
if (a.writeManifest && tax) {
  ...
  const videos = (scope.videos || []).map((v) => {
    ...
    return { id: v.id, title: r.title || v.title, published: r.published || v.published || '', duration_seconds: r.duration_seconds ?? null,
      creator: r.creator || v.creator || '', playlist_positions: v.playlist_positions || [], current: v.current,
      reason: r.reason || null, status: r.status || 'pending', concepts: byVideo[v.id] || [] };
  });
  const manifest = {
    playlist: scope.playlist, playlist_id: scope.playlist_id, playlist_url: scope.playlist_url, entries: scope.entries,
    slug: scope.slug, generated: new Date().toISOString().slice(0, 10),
    generator: 'playlist-to-skill', taxonomy_version: tax.version || 1,
