...
sources.md STRUCTURE: a table in playlist order from scope.json.entries: | Position | Creator | Date | Kind | Title (linked to youtube.com/watch?v=id when an ID exists) | Fetch status | Concepts (linked) |. Account for EVERY entry, including repeated IDs, unavailable entries, failed fetches, missing captions, and videos with zero concepts. Include removed entries retained for fold-in in a separate previous-sources section. Kind is "video" or "short" from ${kbDir}/raw/manifest.json. Mark superseded videos with "(older take)". Use the manifest for dates and titles and taxonomy.json for the concept mapping.
