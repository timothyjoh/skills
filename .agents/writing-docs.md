...
Every skill in a promoted bucket (`knowledge/`, `writing/`, `media/`) has a human-facing **docs page** at `docs/<bucket>/<skill-name>.md`. The page is not the skill and not a copy of `SKILL.md`: it orients one reader around one skill so they know what it is, when to reach for it, and what it looks like when it is working. Non-promoted buckets (`in-progress/`, `deprecated/`) ship no docs page.
